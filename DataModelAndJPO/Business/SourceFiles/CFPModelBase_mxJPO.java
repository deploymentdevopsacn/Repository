...
public class CFPModelBase_mxJPO extends emxDomainObject_mxJPO {
	...
	private static final String cellIdSelectCurrent = "to[" + ManufacturingPlanConstants.RELATIONSHIP_LOGICAL_FEATURES + "].tomid[" + 
			ManufacturingPlanConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].from."+ManufacturingPlanConstants.SELECT_CURRENT;
	private static final String cellIdSelectPolicy = "to[" + ManufacturingPlanConstants.RELATIONSHIP_LOGICAL_FEATURES + "].tomid[" + 
			ManufacturingPlanConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].from."+ManufacturingPlanConstants.SELECT_POLICY;
	private static final String cellIdSelectType = "to[" + ManufacturingPlanConstants.RELATIONSHIP_LOGICAL_FEATURES + "].tomid[" + 
			ManufacturingPlanConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].from."+ManufacturingPlanConstants.SELECT_TYPE;
   ...
   */
				...
				if (iLevelTemp == 1 
					...
					if (strObjectId != null && tempMap.get(cellIdSelect) != null) {
						if (tempMap.get(cellIdSelect) instanceof StringList) {
							...
							StringList listSelectCurrent = (StringList)tempMap.get(cellIdSelectCurrent);
							StringList listSelectPolicy = (StringList)tempMap.get(cellIdSelectPolicy);
							StringList listSelectType = (StringList)tempMap.get(cellIdSelectType);
						    for (int j = 0; j < listSelect.size(); j++) {
						    	...
						    	if ( strReportFormat != null && strReportFormat.length() > 0) {
						    		processCellIdrowMapForReport( context, mapCellIDValues, strobject, tempMap,
							   		...
							   		boolean isFrozen = ManufacturingPlanUtil.isFrozenState(context, listSelectCurrent.get(j).toString(),listSelectPolicy.get(j).toString(),listSelectType.get(j).toString()); 
							...
							if (strReportFormat !=null && strReportFormat.length()>0) {
								...
								boolean isFrozen = ManufacturingPlanUtil.isFrozenState(context, (String)tempMap.get(cellIdSelectCurrent),(String)tempMap.get(cellIdSelectPolicy),(String)tempMap.get(cellIdSelectType)); 
	...
	private MapList AddEditIcontoEmptyCells(Context context,String selectIds,MapList rowList,String objectId,String parentid)throws Exception{
		...
		try{
			...
			StringList columnidsCurrent  = new StringList();
			StringList columnidsPolicy = new StringList();
			StringList columnidsType = new StringList();
			boolean isSelectIds = false ;
			if(selectIds != null && !"".equals(selectIds.trim()) ){
				...
				for (Iterator iterator = tempmapList.iterator(); iterator.hasNext();) {
					...
					columnids.add(objectMap.get(DomainConstants.SELECT_ID).toString());
					columnidsCurrent.add(objectMap.get(DomainConstants.SELECT_CURRENT).toString());
					columnidsPolicy.add(objectMap.get(DomainConstants.SELECT_POLICY).toString());
					columnidsType .add(objectMap.get(DomainConstants.SELECT_TYPE).toString());
				}
			...
			if (columnids !=null){
				for (Iterator iterator = rowList.iterator(); iterator.hasNext();) {
					...
					for (int i = 0; i < columnids.size(); i++) {
						...
						String columnType , columnPolicy , columnCurrent ;
						for (int i = 0; i < columnids.size(); i++) {
							columnid =(String)columnids.get(i);
							columnType = (String) columnidsType.get(i);
							columnPolicy = columnidsPolicy.get(i).toString();
							columnCurrent = columnidsCurrent.get(i).toString();
							if(!cellids.contains(columnid)){
								boolean isFrozen = ManufacturingPlanUtil.isFrozenState(context, columnCurrent , columnPolicy, columnType);
								if(!isFrozen){
									emptyCellvalues.put(columnid, buildIconforemptyCell(buildURL(context,(String)rowMap.get(DomainConstants.SELECT_ID),columnid,parentid), context));
								}
	...
	private MapList getModelDerivationStructure(Context context,String strObjectID, String strSelectId) throws Exception{
		...
		try{
			...
			if (strSelectId != null && !"".equalsIgnoreCase(strSelectId)){
				...
				strListSelectstmts.add(DomainObject.SELECT_TYPE);
				strListSelectstmts.add(DomainObject.SELECT_CURRENT);
				strListSelectstmts.add(DomainObject.SELECT_POLICY);
    ...
    private MapList getModelMasterCompositionStructure(Context context,String strObjectID, String strParentID) throws Exception{
		...
		try {
			...
			objectSelects.add(cellIdSelectCurrent);
			objectSelects.add(cellIdSelectPolicy);
			objectSelects.add(cellIdSelectType);
			objectSelects.add(cellusageAttribute);
			objectSelects.add("attribute["+ManufacturingPlanConstants.ATTRIBUTE_MARKETING_NAME+"]");
			objectSelects.addElement(ManufacturingPlanConstants.SELECT_POLICY);//NN
			objectSelects.addElement(ManufacturingPlanConstants.SELECT_CURRENT);//NN
...
//			stbRelName.append(ManufacturingPlanConstants.RELATIONSHIP_DERIVED);
			...
			String objectWhere = 
				"(to[" + 
					...
					"to[" + ManufacturingPlanConstants.RELATIONSHIP_MODEL_MASTER_FEATURES + "].from.id == " + strParentID + "" +
							")";
   
//				"(to[" +ManufacturingPlanConstants.RELATIONSHIP_DERIVED + "] == True || "+
//				"to[" + 
//					ManufacturingPlanConstants.RELATIONSHIP_PRODUCTS + "].from.to[" + 
//					ManufacturingPlanConstants.RELATIONSHIP_MODEL_MASTER_FEATURES + "].from.id == " + strParentID + " || " +
//					"to[" + 
